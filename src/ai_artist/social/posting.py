...
class TwitterPoster:
    ...
    def post(
        ...
        try:
            ...
            if alt_text:
                self._api.create_media_metadata(
                    media.media_id, alt_text=alt_text[:1000]
                )

            # Post tweet using v2 API
            response = self._client.create_tweet(
                text=text[:280], media_ids=[media.media_id]
            )
...
class BlueskyPoster:
    ...
    def post(
        ...
        try:
            ...
            # Upload image (blob is handled internally by send_image)
            self._client.upload_blob(image_bytes)
...
class SocialPoster:
    ...
    def post_artwork(
        ...
        """Post artwork with auto-generated or caller-supplied caption.
        ...
        Args:
            ...
            caption: Optional full caption (Lumira's voice). When set, used
                across platforms instead of the default template.
        ...
        if caption and caption.strip():
            ...
            if mood:
                caption_text += f" - created while feeling {mood}"
        ...
        if "twitter" in target_platforms and self.twitter.is_available:
            ...
            # Twitter hard limit ~280; keep intent-friendly
            if len(twitter_text) > 270:
                twitter_text = twitter_text[:267] + "…"
        ...
        if "instagram" in target_platforms and self.instagram.is_available:
            results["instagram"] = self.instagram.post(image, caption_text, hashtags)

        if "bluesky" in target_platforms and self.bluesky.is_available:
            bsky_text = (
                caption_text
                if caption
                else (f'"{title}" 🎨 AI-generated artwork by Lumira')
            )
            if len(bsky_text) > 300:
                bsky_text = bsky_text[:297] + "…"
